...
def get_f0m(audio_file_path: str, extract_speeches: bool) -> None:
    if extract_speeches:
        ap = AudioProcessing(audio_file_path)
        ap.extract_speeches()
        ap.join_speech_segments()
        audio = ap.joined_speeches_audio_path
    else:
        audio = audio_file_path

    freq = AudioProcessingUtils.get_fundamental_frequency(audio)
...
def main():
    ...
    f0m_parser.add_argument("--extract-speeches", action=argparse.BooleanOptionalAction,
                            help="Provide this flag if you want to extract speeches and then measure f0m")
    ...
    if args.action == 'get-f0m':
        get_f0m(args.audio_file_path, args.extract_speeches)
